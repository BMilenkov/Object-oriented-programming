...
public:
    ...
    Demonstrator(int indeks, int *ocenki, int brojOcenki,
                 char *imeIPrezime, Kurs *kursevi,
                 int brojKursevi, int brojNaCasovi) : Predavach(imeIPrezime,kursevi,brojKursevi), Student(indeks,ocenki,brojOcenki) {
            this->broj_na_casovi = brojNaCasovi;
    }

    int getBodovi() {
        if(getBrojKursevi()==0)
            throw NoCourseException(indeks);
        return Student::getBodovi() + (20 * broj_na_casovi) / getBrojKursevi();
    }
    void pecati(){
        Student::pecati();
        cout<<": ";
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (casted) {
            for (int j = 0; j < casted->getBrojKursevi(); j++) {
                if ((*casted)[j] == kurs) //interesting trick!!
                    casted->pecati();
            }
        }
    }
